...
module fms
 ...
 contains
  ...
  subroutine propagate(ti , tf) bind(c, name='propagate')
    ...
    allocate(current_traj(n_trajectories()))
    allocate(old_traj(n_trajectories()))
    current_traj  = 0
    old_traj      = 0
    ...
    do i_bat = 1,n_runs
      ...
      c      = init_amplitude(ti, init_method, old_traj(1:n_old), ovrlp)
      time   = ti
      ...
      do while(time <= tf)
        ...
        if(n_old /= n_current .or. .not.all(old_traj == current_traj)) then
          c = update_basis(current_traj(1:n_current), old_traj(1:n_old), c)
        ...
        if(time > ti) then
          ...
          call update_amplitude(time, current_traj(1:n_current), c)
        endif
...
1001 format(' Propagation of all simulations complete using FMS')
  ...
  subroutine retrieve_matrices(time, batch, n, s_r, s_i, t_r, t_i, v_r, v_i, sdt_r, sdt_i, heff_r, heff_i) bind(c, name='retrieve_matrices')
    ...
    if(n /= size(traj)) stop 'n != size(traj) in retrieve_matrices'

    if(allocated(s)) then
      if(size(s, dim=1) /= size(traj)) then
        deallocate(s, t, v, h, sdt, sinv, heff)
      endif
    endif
