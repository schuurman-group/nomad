...
def overlap(double[::1] a1, double[::1] x1, double[::1] p1,
    ...
    cdef double dx, dp, x_cen, p_cen, asum, real_part, imag_part
    ...
    for i in range(n):
        ...
        dp         = p1[i] - p2[i]
        asum       = 2.*(a1[i] + a2[i])
        x_cen      = (a1[i] * x1[i] + a2[i] * x2[i]) / asum 
        p_cen      = (a1[i] * p2[i] + a2[i] * p1[i]) / asum
        real_part += (2.*a1[i]*a2[i]*dx**2 + 0.5*dp**2) / asum
        imag_part += (dp * x_cen - dx * p_cen) 
...
def deldp(double complex S,
    ...
    for i in range(n):
        dx        = x1[i] - x2[i]
        real_part = p1[i] - p2[i] 
        imag_part = a1[i] * (2. * x1[i] - x2[i]) + a2[i] * x2[i]
        delpv[i]  = S * (real_part + I * imag_part) / (2. * (a1[i] + a2[i])) 
...
def deldx(double complex S,
    ...
    for i in range(n):
        ...
        imag_part = -a2[i] * p1[i] + 0.5 * p2[i] * (a2[i] - a1[i])
        delxv[i] = S * (real_part + I * imag_part) / (a1[i] + a2[i])
...
def deld2x(double complex S,
    ...
    for i in range(n):
        ...
        delx2v[i] =  S * (real_part + I * imag_part) / (a1[i] + a2[i])**2 
