...
def v_integral(traj1, traj2=None, centroid=None, S_ij=None):
    ...
    # elecronic states
        ...
        v = np.dot(fij, traj1.deldx_m(traj2, S_ij=1.))
...
def sdot_integral(traj1, traj2, S_ij=None):
    ...
    of traj1 and traj2. This isn't ideal, but will do for now.
    """
    sdot = (np.dot( traj2.velocity(), traj1.deldx(traj2, S_ij=1.) ) +
            np.dot( traj2.force()   , traj1.deldp(traj2, S_ij=1.) ) +
            1j * traj2.phase_dot()) * traj1.h_overlap(traj2)
    return sdot
